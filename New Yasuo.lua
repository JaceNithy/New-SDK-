...
function CompYasuo:EveMenus()
    ...
    self.CQ3 = self:MenuBool("Combo Q3", true)
...
function CompYasuo:OnDrawMenu()
	if not Menu_Begin(self.menu) then return end
		if (Menu_Begin("[Q] in Combo")) then
            ...
            self.CQ3 = Menu_Bool("Combo Q3", self.CQ3, self.menu)
...
function CompYasuo:CastQ3()
    for i ,enemys in pairs(self:GetEnemies()) do
        local enemys = GetTargetSelector(self.Q3.Range)
        target = GetAIHero(enemys)
        if target ~= 0 then
            if GetSpellNameByIndex(myHero.Addr, _Q) == "YasuoQ3W" then
            if self.CQ and sReady(_Q) and GetDistance(target) < self.Q3.Range then
...
function CompYasuo:CastE()
    for i ,enemys in pairs(self:GetEnemies()) do
    local enemys = GetTargetSelector(1200)
    target = GetAIHero(enemys)
    if target ~= 0 then
    if sReady(_E) then 
    ...
    if sReady(_E) then
        if GetDistance(target) < self.E.Range and not self:Marked(target) then
...
function CompYasuo:AutoR()
    for i ,enemys in pairs(self:GetEnemies()) do
        ...
        target = GetAIHero(enemys)
        if target ~= 0 then
        if GetDistance(target) < 1400 and self.R:GetDamage(target)+self.Q:GetDamage(target) > target.HP then
